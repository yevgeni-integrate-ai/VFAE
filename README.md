...
The variational piece describes the use of variational Bayes techniques to approximate the posterior the model is trying to predict.

Most interesting, and novel, is the fair component. In VFAE we explicitly give information about the properties of the data we wish to avoid have influence our models. The latent version of our dataset generated by this model contains much of the valuable details from the original dataset, but without any influence of the sensitive information. We are then able to train models on this latent representation, and obtain good predictions that are not biased or influenced by sensitive properties of the people included in our datasets.

In our models we will use neural networks as the encoding and decoding pieces.
...
Given the sensitivity of the task, we choose to optimize for precision in correctly classifying those who should be arrested. This would come at the expense of missing a few people who do, but we are OK with that.
We want to simultaneously make sure that we don't miss any people that should be arrested, and avoid incorrectly classifying people that should not.  Unfortunately, this dataset is highly imbalanced, with only about 0.6% of cases being arrests. Furthermore, it does not include many features (26 overall, with 16 being either of no use or redundant). Although the number of rows is quite large at about six million.

If we actually wanted to use this data in real life, we would likely supplement it with external data. However, for the sake of this demonstration, we will artificially reduce the imbalance of the dataset. 

The first thing to do is to clean up the features. The following table lists all the features in the raw dataset, whether the feature will be kept, reasons for doing so:
...
driver_race | Yes | An important feature
...
We take the remaining columns, which all contain categorical data, and replace null values with values taken from a probability distribution of the other values in the column. We then create dummy versions of every column.

The result is a data frame with 82 columns, which is now ready to be analyzed using a machine learning algorithm.  

As mentioned earlier, we take this data frame and reduce the imbalance of positive to negative results. We make sure to do so in a manner which is random. 
...
We see that we still obtain similar performance as before, however, this time the model does not pick up the drivers races, and hence does not result in bias with respect to race.
